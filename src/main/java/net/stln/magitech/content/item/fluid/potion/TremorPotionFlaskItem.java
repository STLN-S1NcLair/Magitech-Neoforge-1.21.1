...
public class TremorPotionFlaskItem extends DrinkableFlaskItem {

    public TremorPotionFlaskItem(Properties properties) {
        super(properties);
    }

    @Override
    protected void applyEffects(Level level, LivingEntity entity, ItemStack stack) {
        entity.addEffect(new MobEffectInstance(MobEffectInit.TREMOR_POWER, 2400, 0));
        EffectHelper.entityEffect(level, new PowerupParticleEffect(new Vector3f(0.3F, 1.0F, 1.0F), new Vector3f(0.0F, 0.7F, 1.0F), 1F, 1, 0, 15, 1.0F), entity, 20);
    }
}
