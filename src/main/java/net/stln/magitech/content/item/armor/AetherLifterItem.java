            ...
            if (level.isClientSide) {
                ...
                for (int i = 0; i < 10; i++) {
                    if (player.getRandom().nextBoolean()) {
                        PointVFX.burst(level, position, Element.HOLLOW, SquareParticles::squareParticle, 1, 0.1F);
                    } else {
                        PointVFX.burst(level, position, Element.PHANTOM, SquareParticles::squareParticle, 1, 0.1F);
                    }
                }
                for (int i = 0; i < position.distanceTo(surface) * 5; i++) {
                    Vec3 lerped = surface.lerp(position, i / position.distanceTo(surface) / 5);
                    if (player.getRandom().nextBoolean()) {
                        PointVFX.burst(level, lerped, Element.HOLLOW, SquareParticles::squareParticle, 1, 0.1F);
                    } else {
                        PointVFX.burst(level, lerped, Element.PHANTOM, SquareParticles::squareParticle, 1, 0.1F);
                    }
                }
                ...
                for (int i = 0; i < 10; i++) {
                    for (int j = 0; j < 3; j++) {
                        TickScheduler.schedule(i + 1, () -> {
                            ...
                            if (player.getRandom().nextBoolean()) {
                                PointVFX.burst(level, pos, Element.HOLLOW, SquareParticles::squareParticle, 1, 0.0F);
                            } else {
                                PointVFX.burst(level, pos, Element.PHANTOM, SquareParticles::squareParticle, 1, 0.0F);
                            }
                        }, level.isClientSide);
                    }
                }
