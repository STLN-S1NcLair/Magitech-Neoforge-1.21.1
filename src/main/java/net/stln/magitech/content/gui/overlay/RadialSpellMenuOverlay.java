...
public class RadialSpellMenuOverlay extends Screen {
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        if (!Minecraft.getInstance().options.hideGui) {
            ...
            double dx = mouseX - x;
            double dy = mouseY - y;
            double naturalMouseAngle = MathHelper.getGeneralAngle(Math.atan2(dy, dx) + Math.PI / 2);
            ...
            for (Spell spell : spellComponent.spells()) {
                ...
                double min = MathHelper.getGeneralAngle(Math.PI * 2 * (index - 0.5) / spellComponent.spells().size());
                double max = MathHelper.getGeneralAngle(Math.PI * 2 * (index + 0.5) / spellComponent.spells().size());
                if (index == 0) {
                    min -= Math.PI * 2;
                    if (mouseAngle > max) {
