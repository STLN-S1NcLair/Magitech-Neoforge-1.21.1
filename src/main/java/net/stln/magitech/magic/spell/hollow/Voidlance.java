...
public class Voidlance extends Spell {
            ...
            for (int j = -1; j < 2; j++) {
                for (int k = -1; k < 2; k++) {
                    if (Math.abs(i) + Math.abs(j) + Math.abs(k) != 1) {
                        ...
                        Vec3 hit = EntityUtil.raycast(user, (double) 24 / Math.pow(order, 2), hitPos.add(dir.scale(0.5)), dir);
                        Entity target = EntityUtil.raycastEntity(user, (double) 24 / Math.pow(order, 2), hitPos.add(dir.scale(0.5)), dir);
                    Magitech.LOGGER.info(dir.toString());
