    ...
    public void onBreakBlock(Player player, Level level, ItemStack stack, int traitLevel, ToolStats stats, BlockState blockState, BlockPos pos, int damageAmount, boolean isInitial) {
        ...
        if (!isInitial) {
            for (int i = 0; i < 20; i++) {
                ...
                level.addParticle(new FrostParticleEffect(new Vector3f(1.0F, 1.0F, 1.0F), new Vector3f(1.0F, 1.0F, 1.0F), 1F, 1, rotSpeed, level.random.nextInt(50, 60), 0.99F),
        ...
        if (target instanceof LivingEntity livingEntity) {
            if (livingEntity.getPercentFrozen() > 0.8F) {
                ...
                for (Entity entity : list) {
                    ...
                    EffectHelper.entityEffect(level, new FrostParticleEffect(new Vector3f(1.0F, 1.0F, 1.0F), new Vector3f(1.0F, 1.0F, 1.0F), 2F, 1, 0.01F, level.random.nextInt(50, 60), 0.99F), entity, 60);
                }
                for (int i = 0; i < 60; i++) {
                    float rotSpeed = player.getRandom().nextFloat() / 5 - 0.1F;
                ...
                EffectHelper.entityEffect(level, new FrostParticleEffect(new Vector3f(1.0F, 1.0F, 1.0F), new Vector3f(1.0F, 1.0F, 1.0F), 2F, 1, rotSpeed, level.random.nextInt(50, 60), 0.99F), livingEntity, 60);
            }
        }
    }
