...
public class Arclume extends Spell {
    ...
    public void use(Level level, Player user, InteractionHand hand, boolean isHost) {
        ...
        Vec3 hitPos = EntityHelper.raycast(user, getDamage(user, this.getRequiredMana(level, user, stack), (float) baseMaxRange, this.getElement())).subtract(forward.scale(user.getBbWidth() / 2 + 0.1));
    ...
    private void addLightning(ItemStack stack, Player user, Level level, Vec3 pos) {
        ...
        List<Entity> entities = EntityHelper.getEntitiesInBox(level, user, surface, new Vec3(2, 2, 2));
