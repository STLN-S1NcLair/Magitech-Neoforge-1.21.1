package net.stln.magitech.feature.magic.spell.glace;
...
import net.stln.magitech.vfx.particle.particle_option.FrostParticleEffect;
...
public class Glistelda extends Spell {
    ...
    public void use(Level level, Player user, InteractionHand hand, boolean isHost) {
        ...
        if (!level.isClientSide) {
            user.addEffect(new MobEffectInstance(MobEffectInit.LEAP_STEP, duration, 6, false, false, true));
        }
        ...
        for (int i = 0; i < duration; i++) {
            TickScheduler.schedule(i, () -> {
                List<Entity> nearbyEntities = EntityHelper.getEntitiesInBox(level, user, user.position(), new Vec3(3, 3, 3));
                entities.addAll(nearbyEntities);
                EffectHelper.entityEffect(level, new FrostParticleEffect(new Vector3f(1.0F, 1.0F, 1.0F), new Vector3f(1.0F, 1.0F, 1.0F), 1.0F, 1, 0, level.random.nextInt(50, 60), 0.99F), user, 4);
                EffectHelper.entityEffect(level, new SquareParticleEffect(new Vector3f(1.0F, 1.0F, 1.0F), new Vector3f(0.6F, 1.0F, 1.0F), 1.0F, 1, 0, 15, 1.0F), user, 10);
                for (Entity entity : nearbyEntities) {
                    EffectHelper.entityEffect(level, new FrostParticleEffect(new Vector3f(1.0F, 1.0F, 1.0F), new Vector3f(1.0F, 1.0F, 1.0F), 1.0F, 1, 0, level.random.nextInt(50, 60), 0.99F), entity, 1);
