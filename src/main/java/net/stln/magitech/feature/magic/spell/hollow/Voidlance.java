package net.stln.magitech.feature.magic.spell.hollow;
...
public class Voidlance extends BeamSpell {
        ...
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                for (int k = -1; k < 2; k++) {
                    ...
                    Vec3 hit = EntityHelper.raycastBeam(user, (double) 24 / Math.pow(order, 2), hitPos.add(dir.scale(0.5)), dir, beamradius);
                    Entity target = EntityHelper.raycastBeamEntity(user, (double) 24 / Math.pow(order, 2), hitPos.add(dir.scale(0.5)), dir, beamradius);
                    Vec3 start = hitPos.add(dir.scale(0.5));
                    EffectHelper.lineEffect(level, new VoidGlowParticleEffect(new Vector3f(1.0F, 1.0F, 1.0F), new Vector3f(1.0F, 1.0F, 1.0F), 1.0F, 1, 0, level.random.nextInt(1, 21), 1.0F), start, hit, 2, false);
                    level.addParticle(new BeamParticleEffect(new Vector3f(0.3F, 0.0F, 1.0F), new Vector3f(0.5F, 0.0F, 1.0F), hit.toVector3f(), 0.7F, 1, 1, 5, 1), start.x, start.y, start.z, 0, 0, 0);
                    for (int l = 0; l < 20; l++) {
    ...
    protected void addVisualEffect(Level level, Player user, Vec3 start, Vec3 hitPos) {
        EffectHelper.lineEffect(level, new VoidGlowParticleEffect(new Vector3f(1.0F, 1.0F, 1.0F), new Vector3f(1.0F, 1.0F, 1.0F), 1.0F, 1, 0, level.random.nextInt(1, 21), 1.0F), start, hitPos, 2, false);
        level.addParticle(new BeamParticleEffect(new Vector3f(0.3F, 0.0F, 1.0F), new Vector3f(0.5F, 0.0F, 1.0F), hitPos.toVector3f(), 0.7F, 1, 1, 5, 1), start.x, start.y, start.z, 0, 0, 0);
        for (int i = 0; i < 20; i++) {
            level.addParticle(new VoidGlowParticleEffect(new Vector3f(1.0F, 1.0F, 1.0F), new Vector3f(1.0F, 1.0F, 1.0F), 1.0F, 1, 0, level.random.nextInt(1, 21), 1.0F),
