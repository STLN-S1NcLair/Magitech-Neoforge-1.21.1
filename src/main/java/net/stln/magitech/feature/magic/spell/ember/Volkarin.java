...
public class Volkarin extends Spell {
    ...
    public Map<ManaUtil.ManaType, Double> getBaseRequiredMana() {
        ...
        cost.put(ManaUtil.ManaType.FLUXIA, 8.0);
        return cost;
    }
        ...
        if (livingEntity instanceof Player user) {
            if (ChargeData.getCurrentCharge(user) == null && timeCharged > 1 && ManaUtil.useManaServerOnly(user, this.getRequiredMana(level, user, stack))) {
                ...
                if (!level.isClientSide && !isHost) {
                    VolkarinEntity bullet = new VolkarinEntity(level, user, stack, getDamage(user, this.getCost(level, user, stack), baseDamage, this.getElement()));
