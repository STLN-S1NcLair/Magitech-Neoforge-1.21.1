...
public class IllusflareRenderer extends GeoEntityRenderer<IllusflareEntity> {
    ...
    @Override
    public Color getRenderColor(IllusflareEntity animatable, float partialTick, int packedLight) {
        return Color.WHITE;
    }

    @Override
    public @Nullable RenderType getRenderType(IllusflareEntity animatable, ResourceLocation texture, @Nullable MultiBufferSource bufferSource, float partialTick) {
        return RenderHelper.additiveNoCull(texture);
    }

    @Override
    protected void applyRotations(IllusflareEntity animatable, PoseStack poseStack, float ageInTicks, float rotationYaw, float partialTick, float nativeScale) {
