...
public class HexflareRenderer extends GeoEntityRenderer<HexflareEntity> {
    ...
    @Override
    public Color getRenderColor(HexflareEntity animatable, float partialTick, int packedLight) {
        return Color.WHITE;
    }

    @Override
    public @Nullable RenderType getRenderType(HexflareEntity animatable, ResourceLocation texture, @Nullable MultiBufferSource bufferSource, float partialTick) {
        return RenderHelper.additiveNoCull(texture);
    }

    @Override
    protected void applyRotations(HexflareEntity animatable, PoseStack poseStack, float ageInTicks, float rotationYaw, float partialTick, float nativeScale) {
