        ...
        this.add(EntityInit.WEAVER_ENTITY.get(),
                LootTable.lootTable()
                        .withPool(LootPool.lootPool()
                                ...
                                .when(TimeCheck.time(IntRange.range(2000, 10000)))
                                .add(LootItem.lootTableItem(ItemInit.AGGREGATED_LUMINIS.get()))
                                .apply(EnchantedCountIncreaseFunction.lootingMultiplier(this.registries, UniformGenerator.between(0.0F, 1.0F)))
                        )
                        .withPool(LootPool.lootPool()
                                .setRolls(ConstantValue.exactly(1))
                                .when(TimeCheck.time(IntRange.range(10000, 14000)).or(TimeCheck.time(IntRange.range(22000, 24000))).or(TimeCheck.time(IntRange.range(0, 2000))))
                                .add(LootItem.lootTableItem(ItemInit.AGGREGATED_FLUXIA.get()))
                                .apply(EnchantedCountIncreaseFunction.lootingMultiplier(this.registries, UniformGenerator.between(0.0F, 1.0F)))
                        )
                        .withPool(LootPool.lootPool()
                                .setRolls(ConstantValue.exactly(1))
                                .when(TimeCheck.time(IntRange.range(14000, 22000)))
